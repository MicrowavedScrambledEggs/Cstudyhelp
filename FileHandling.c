/*An extension of BinaryPacking.c Still about the under $10 store. This time it
  allows the user to save the product array to a file, and loads the file when
  opened.
  
  Missing deletion. Feel free to add this
...
void load_inventory(int products[]);
void save_inventory(int products[]);
	...
	while(choice != NUM_OP - 1){
		...
		switch(choice){
			...
			case(2):print_inventory(products); 
					break;
			case(3):save_inventory(products);
					break;
		}
	}
...
void load_inventory(int products[]){
	...
	if(data == NULL){
		printf("\nUnable to load data (This should appear when running the program for the first time)\n");
		//initialises product ints to 0
		for(i = 0; i < INVENTORY_SIZE; i++)
			products[i] = 0;
	} else {
		printf("\nLoaded %d products\n", fread(products, sizeof(int), INVENTORY_SIZE, data));
	}
	fclose(data);
}
...
void print_menu(){
	...
	int i;
	char *options[NUM_OP] = {"Add Product", "Find Product", "Print Inventory",
		"Save Inventory", "Exit"};
...
void add_product(int products[]){
	...
	if(products[INVENTORY_SIZE -1] != 0){
		printf("\nInventory Full ;_;\n");
		return;
	}
...
int product_exists(int id, int products[]){
	...
	for(i = 0; i < INVENTORY_SIZE; i++){
		if(products[i] == 0) return 0;//gets to an empty cell
...
void save_inventory(int products[]){
	...
	if(data == NULL) fprintf(stderr, "Error when saving inventory\n");
	else printf("\nWriting %d products\n", fwrite(products, sizeof(int), INVENTORY_SIZE, data));
	fclose(data);
}
